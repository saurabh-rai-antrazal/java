...
public class ZoneRepo {
    ...
    public static String addNewZone(Zone zone, int zooId){
        try {
            ...
            pstmt = conn.prepareStatement(Const.FIND_MAX_ZONES_BY_ZOO_ID);
            ...
            rs.close(); 
            pstmt.close();
            pstmt = conn.prepareStatement(Const.COUNT_ZONES_BY_ZOO_ID);
            pstmt.setInt(1, zooId);
            rs = pstmt.executeQuery();
            int count = 0;
            if (rs.next()) {
            ...
            if (count >= maxZones) {
                return Const.ERROR_INSUFFICIENT_SPACE_FOR_ZONE;
            }
            pstmt = conn.prepareStatement(Const.INSERT_NEW_ZONE);
            pstmt.setInt(1, zooId);
            pstmt.setString(2, zone.getZoneCategory());
            pstmt.setBoolean(3, zone.isHasPark());
            pstmt.setBoolean(4, zone.isHasCanteen());
            pstmt.executeUpdate();
            return Const.SUCCESS_ZONE_CREATED;
